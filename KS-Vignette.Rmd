...
D
```
Our calculated value of D is 0.133 which is smaller than our critical value of 0.99. This also makes sense, and confirms our ability to reject the null.

#Challenge 2
Using a two-sample KS test, compare age and weight (n=2000)
```{r 15, include=TRUE}
#using our function, we are going to fill both arguments with the data sets to ensure a comparison
height <- z$height
weight <- z$weight
ks.test(weight, height)
```